...
class P3PlayerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("P3 File Player")
        ...
        while True:
            ...
            if self.current_index >= len(self.playlist):
                if self.loop_playback.get():  # If loop playback is enabled
                    self.current_index = 0  # Back to first track
                else:
            ...
            if not self.loop_playback.get():  # If loop playback is not enabled
                break  # Stop after current file finishes
    ...
    def remove_files(self):
        ...
        if not selected_indices:
            messagebox.showwarning("Warning", "Please select files to remove first!")
            return
