    ...
    if (is_edarobot_resting_ == false) {  // Go to rest position only if necessary
        ...
        for (int i = 0; i < SERVO_COUNT; i++) {
            if (i == LEFT_HAND || i == RIGHT_HAND) {
                if (hands_down) {
                    // Reset hands to default if requested
                    if (i == LEFT_HAND) {
                    ...
                    // Keep current position if hands should not reset
...
//--    * amount: hand swing amount; 0 means no swing
//---------------------------------------------------------
void EdaRobot::Walk(float steps, int period, int dir, int amount) {
    ...
    // If amount>0 and hand servos exist, set hand amplitude and phase
    if (amount > 0 && has_hands_) {
        // Arm amplitude uses the amount parameter
        A[LEFT_HAND] = amount;
        A[RIGHT_HAND] = amount;

        // Left hand in phase with right leg, right hand with left leg for natural gait
...
void EdaRobot::Turn(float steps, int period, int dir, int amount) {
    ...
    if (amount > 0 && has_hands_) {
        ...
        // Turning arm phase: left hand with left leg, right hand with right leg
