...
private:
    ...
    void CheckBatteryStatus() {
        ...
        // Read battery level when not enough samples
        if (adc_values_.size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData();
            return;
        }

        // When enough samples exist, read battery every kBatteryAdcInterval ticks
        ticks_++;
...
public:
    ...
    bool IsCharging() {
        ...
        if (battery_level_ == 100) {
            //ESP_LOGI(TAG, "Battery full, not showing charging");
            return false;
        }
        return is_charging_;
    }
    ...
    bool IsDischarging() {
        // Charging/discharging not distinguished; return inverted state
        return !is_charging_;
    }
    ...
    void RegisterAllCallbacks() {
        // Register power state change callback (optimized)
        power_controller_->OnStateChange([this](PowerState newState) {
            switch(newState) {
                ...
                // Disable PWR_EN
                    /* Prevent accidental wake after shutdown */
                    ESP_ERROR_CHECK(esp_sleep_enable_ext0_wakeup(PWR_BUTTON_GPIO, 0));
                    ESP_ERROR_CHECK(rtc_gpio_pulldown_en(PWR_BUTTON_GPIO)); // Internal pulldown
