...
private:
    ...
    bool adc_handle_owned_ = false;  // Whether this class owns the ADC handle
    adc_cali_handle_t adc_cali_handle_ = nullptr;  // ADC calibration handle
    ...
    void CheckBatteryStatus() {
        ...
        // Read battery level when insufficient samples
        if (adc_values_.size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData();
            return;
        }

        // When enough samples, read every kBatteryAdcInterval ticks
        ticks_++;
    ...
    void ReadBatteryAdcData() {
        int adc_raw = 0;
        int voltage_mv = 0;  // Calibrated voltage (mV)
        ...
        // Use ADC calibration for accurate voltage
        if (adc_cali_handle_) {
            ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc_cali_handle_, adc_raw, &voltage_mv));
        } else {
            // Without calibration, use linear calculation
            voltage_mv = (int)(adc_raw * 3300.0f / 4095.0f);
        }
        
        // Compute battery voltage from divider ratio
        // Divider: R21/(R20+R21) = 100K/300K = 1/3
        // Battery voltage = ADC voltage × 3
        int battery_voltage_mv = voltage_mv * 3;
        
        // Add voltage to queue for smoothing
        ...
        // Battery level ranges (actual battery voltage in mV)
        const struct {
            uint16_t voltage_mv;  // Battery voltage (mV)
            uint8_t level;        // Level percent
        } levels[] = {
            {3500, 0},    // 3.5V
            {3640, 20},   // 3.64V
            {3760, 40},   // 3.76V
...
public:
    // Constructor with external ADC handle (reuse)
    PowerManager(gpio_num_t pin, adc_oneshot_unit_handle_t* external_adc_handle = nullptr) 
        ...
        if (external_adc_handle != nullptr && *external_adc_handle != nullptr) {
            ...
            // Create new ADC handle
            adc_oneshot_unit_init_cfg_t init_config = {
                .unit_id = ADC_UNIT_1,  // GPIO6 maps to ADC1
    ...
    ~PowerManager() {
        ...
        // Delete ADC handle only if owned by this class
    ...
    bool IsCharging() {
        // Do not show charging when full
    ...
    bool IsDischarging() {
        // No charge/discharge distinction; return inverted state
        return !is_charging_;
    }
