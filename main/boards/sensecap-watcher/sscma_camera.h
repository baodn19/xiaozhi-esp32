...
private:
    ...
    enum DetectionState {
        IDLE,           // Idle
        VALIDATING,     // Validating (continuous detection 3s)
        COOLDOWN        // Cooldown (wait before re-detect)
    };
    
    DetectionState detection_state = IDLE;
    int64_t state_start_time = 0;
    bool need_start_cooldown = false; // Whether cooldown should start
    int64_t last_detected_time = 0; // Last detection time during validation
    
    int detect_target = 0;
    int detect_threshold = 75;
    int detect_duration_sec = 2; // Detection duration 2s to confirm persistent presence
    int detect_invoke_interval_sec = 8; // Default 8s cooldown to avoid frequent sessions
    int detect_debounce_sec = 1; // 1s debounce when person leaves during validation
    int inference_en = 0; // Inference enable (0: off, 1: on)
