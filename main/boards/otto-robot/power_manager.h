...
class PowerManager {
private:
    // Battery levels - voltage divider uses two 100k resistors
    ...
    inline static bool battery_update_paused_ = false;  // Static flag: battery update paused

    adc_oneshot_unit_handle_t adc_handle_;

    void CheckBatteryStatus() {
      // Skip update when battery updates are paused (action in progress)
...
public:
    PowerManager(gpio_num_t charging_pin, adc_unit_t adc_unit = ADC_UNIT_2,
      ...
      if (charging_pin_ != GPIO_NUM_NC) {
        ...
        ESP_LOGI("PowerManager", "Charging detect pin not configured; skipping charging state detection");
      }
    ...
    // Pause/resume battery updates during actions
