...
static uint8_t* convert_input_to_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    // GRAY can be fed directly as JPEG_PIXEL_FORMAT_GRAY
    ...
    // V4L2 YUYV (Y Cb Y Cr) can be fed directly as JPEG_PIXEL_FORMAT_YCbYCr
    ...
    // V4L2 UYVY (Cb Y Cr Y) -> reorder to YUYV, then feed as YCbYCr
    // UYVY is not used in the current version
    ...
    // YUV422P is not used in the current version
    ...
    // Convert RGB to YUV422 (YCbYCr) before encoding
    // See https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
        ...
        switch (format) {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                break;
            [[unlikely]] case V4L2_PIX_FMT_RGB565X: // RGB565X is not used in the current version
...
static uint8_t* convert_input_to_hw_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    if (format == V4L2_PIX_FMT_YUYV) {
        // Hardware expects big-endian | Y1 V Y0 U |, so bswap16 is required
        int sz = (int)width * (int)height * 2;
