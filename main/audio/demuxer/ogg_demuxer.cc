        ...
        switch (state_) {
          case ParseState::FIND_PAGE: {
            ...
            if (ctx_.bytes_needed < 4) {
                // Handle incomplete "OggS" match (across data chunks)
                ...
                if (ctx_.bytes_needed == 0) {
                    // Check whether "OggS" matches
                    if (memcmp(ctx_.header, "OggS", 4) == 0) {
                        state_ = ParseState::PARSE_HEADER;
                        ctx_.data_offset = 4;
                        ctx_.bytes_needed = 27 - 4;  // Need 23 more bytes to complete the page header
                    } else {
                        // Match failed; slide 1 byte and continue matching
                ...
                // Search for complete "OggS" in the data chunk
                bool found = false;
                size_t i = 0;
                ...
                if (found) {
                    // Found "OggS"; skip searched bytes
                    processed += i;
                    ...
                    // Complete "OggS" not found; save possible partial match
                    ...
                    if (partial_len > 0) {
                        ...
                        processed += i;  // Searched all bytes
                    }
          ...
          case ParseState::PARSE_HEADER: {
            ...
            if (available < ctx_.bytes_needed) {
                // Not enough data; copy available portion
          ...
          case ParseState::PARSE_DATA: {
            while (ctx_.seg_index < ctx_.seg_count && processed < size) {
                ...
                // Check whether segment data was partially read already
                ...
                if (!seg_continued) {
                    ...
                    if (ctx_.packet_len) {
                        ...
                        if (opus_info_.head_seen && opus_info_.tags_seen) {
                            ...
                            ESP_LOGW(TAG, "OpusHead/OpusTags not parsed in current Ogg container; discarding");
                        }
                    }
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
            ...
            if (ctx_.seg_index == ctx_.seg_count) {
                // Check whether the entire body has been read
